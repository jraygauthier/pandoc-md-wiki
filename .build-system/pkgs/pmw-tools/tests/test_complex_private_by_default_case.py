...
from pmw_tools.yaml_wiki_walker import (categorize_wiki_pages,
                                        categorize_wiki_pages_json_ready)
from test_lib.fixture_paths import get_test_case_dir
...
def _find_should_be_line(page_path: Path) -> str:
...
def _parse_should_be_set(page_path: Path) -> Set[str]:
    should_be_sstr = get_should_be_substr()
    known_tags = ["private", "public", "work"]
    shoud_be_ln = _find_should_be_line(page_path)
    try:
...
def test_no_root_tags_case():
    root_dir = get_test_case_dir("NoRootTagsCase")
    out = categorize_wiki_pages(root_dir)
...
def test_categorize_wiki_pages_all_cases(
        case_name: str, expected_known_tags: Set[str]):
    root_dir = get_test_case_dir(case_name)
    out = categorize_wiki_pages(root_dir)
    ...
    for md_p in all_md_paths:
        expected_tags = _parse_should_be_set(md_p)
